...
# %% 
def ak_mcs(S, G, learning_fun, N1=12, k=0, std=False):
    """
    ...
    # A PARTIR DE LAS PDFS generar aqui adentro los puntos
    ...
    ###########################################################################
            # store the values
            results = np.row_stack((results, [[n_MC, lf_x[idx_x_ast], pf_hat]]))
            if plot_9grid:
                if (i % k) == 0:
